(function() {
    ...
    function mostrarNotificacao(cor, forca, padrao) {
        ...
        notif.innerHTML = `
            ...
            <div class="notificacao-conteudo"><img src="${logoUrl}" class="logo-sinal" style="background:transparent;"><div><div class="notificacao-cor ${cor}">${cor === 'red' ? '🔴' : '⚫'} ${corNome}</div><div>📊 ${padrao}</div></div></div>
    ...
    function mostrarNotificacaoWin(cor, martingale) {
        ...
        let galeText = martingale === 0 ? '✅ 1ª ENTRADA' : (martingale === 1 ? '🔄 GALE 1' : '🔄🔄 GALE 2');
        const notif = document.createElement('div');
        notif.className = 'notificacao-sinal notificacao-win';
        notif.innerHTML = `<div class="notificacao-header"><span class="notificacao-titulo">✅ VITÓRIA!</span><button class="notificacao-fechar">&times;</button></div><div class="notificacao-conteudo"><img src="${logoUrl}" class="logo-sinal"><div><div class="notificacao-cor ${cor}">${corIcone} ${corTexto}</div><div>${galeText}</div><div style="color:#00ff00;">🎉 WIN!</div></div></div><div class="notificacao-timer"><div class="timer-bar"></div></div>`;
        document.body.appendChild(notif);
        setTimeout(() => notif.classList.add('mostrar'), 10);
        notif.querySelector('.notificacao-fechar').onclick = () => notif.remove();
